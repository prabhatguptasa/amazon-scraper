...
class BodyButtersSpider(scrapy.Spider):
    name = 'body_butter'
    custom_settings = {
        'LOG_ENCODING': 'UTF-8',
    ...
    def start_requests(self):
        urls = ['https://www.amazon.in/s?i=beauty&bbn=1374408031&rh=n%3A1355016031%2Cn%3A%211355017031%2Cn%3A1374407031%2Cn%3A1374408031%2Cn%3A1374409031%2Cp_85%3A10440599031&pf_rd_i=1374407031&pf_rd_m=A1VBAL9TL5WCBF&pf_rd_p=217dbcc0-819b-439f-ad72-601b8fd88630&pf_rd_r=PJH8N2A7CTX3D4Y3BHXY&pf_rd_s=merchandised-search-9&ref=QANav11CTA_en_IN_5']

        """i = 1
        while i < 1:
            i += 1
            urls.append(f'https://www.amazon.in/s?i=beauty&bbn=1374408031&rh=n%3A1355016031%2Cn%3A1355017031%2Cn%3A1374407031%2Cn%3A1374408031%2Cn%3A1374409031%2Cp_85%3A10440599031&page={i}&pf_rd_i=1374407031&pf_rd_m=A1VBAL9TL5WCBF&pf_rd_p=217dbcc0-819b-439f-ad72-601b8fd88630&pf_rd_r=PJH8N2A7CTX3D4Y3BHXY&pf_rd_s=merchandised-search-9&qid=1615836180&ref=sr_pg_{i}')
"""
            ...
            for feature in response.xpath('//div[@id="detailBullets_feature_div"]//ul//li'):
                if 'Customer Reviews:' not in feature.xpath('.//span[@class="a-text-bold"]/text()').extract_first():
                    if 'Best Sellers Rank' not in feature.xpath('.//span[@class="a-text-bold"]/text()').extract_first():
                        row[feature.xpath('.//span[@class="a-text-bold"]/text()').extract_first().replace('\n\n\n\n:\n\n\n', '')] = feature.xpath('.//span[@class="a-text-bold"]//following::span/text()').extract_first()
        except:
            pass
